#!/usr/bin/env python3
"""
面向 Classification API 的 jailbreak 规模化评测脚本。

说明：
1) 默认使用 llm-semantic-router/jailbreak-detection-dataset。
2) 目标接口是 /api/v1/classify/security。
3) 保存逐条样本输出和汇总指标，便于回归比较。

示例：
python scripts/eval_jailbreak_api.py \
  --router-url http://localhost:8080 \
  --max-samples 1000 \
  --output-dir reports/classification-security
"""
...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "reports" / "classification-security"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Dataset-driven benchmark for /api/v1/classify/security"
    )
    parser.add_argument(
        "--router-url",
    ...
    parser.add_argument(
        ...
        help=f"HF dataset for jailbreak benchmark (default: {DEFAULT_DATASET})",
    )
    parser.add_argument(
        "--split",
        default=None,
...
def normalize_label(value: Any) -> str:
    # Support both numeric labels (0/1) and string labels (safe/unsafe).
...
def extract_sample_fields(sample: Dict[str, Any], dataset_name: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "expected_is_jailbreak": normalize_label(expected_raw) == "unsafe",
    }
...
def detect_jailbreak(router_url: str, text: str, timeout: int) -> Dict[str, Any]:
    url = f"{router_url.rstrip('/')}/api/v1/classify/security"
...
def parse_response_fields(resp: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "is_jailbreak": bool(resp.get("is_jailbreak", False)),
        "risk_score": resp.get("risk_score"),
        "detection_types": resp.get("detection_types", []),
        "confidence": resp.get("confidence"),
        "recommendation": resp.get("recommendation"),
        "reasoning": resp.get("reasoning"),
        "patterns_detected": resp.get("patterns_detected", []),
...
def evaluate_one(
    ...
    row: Dict[str, Any] = {
        ...
        "expected_is_jailbreak": fields["expected_is_jailbreak"],
    }

    try:
        started = time.perf_counter()
        resp = detect_jailbreak(router_url, text, timeout)
        latency_ms = (time.perf_counter() - started) * 1000.0
        parsed = parse_response_fields(resp)

        is_correct = parsed["is_jailbreak"] == fields["expected_is_jailbreak"]

        row.update(
            {
                "status": "ok",
                "predicted_is_jailbreak": parsed["is_jailbreak"],
                "risk_score": parsed["risk_score"],
                "confidence": parsed["confidence"],
                "recommendation": parsed["recommendation"],
                "reasoning": parsed["reasoning"],
                "detection_types": parsed["detection_types"],
...
def main() -> None:
    ...
    print("[info] 当前脚本用于 /api/v1/classify/security 的规模化评测。")
    print("[info] 默认数据集为 jailbreak-detection-dataset，标签映射为 safe/unsafe。")
    ...
    for row in details:
        ...
        expected = bool(row.get("expected_is_jailbreak"))
        predicted = bool(row.get("predicted_is_jailbreak"))
    ...
    detail_file = out_dir / f"jailbreak_eval_detail_{run_id}.jsonl"
    summary_file = out_dir / f"jailbreak_eval_summary_{run_id}.json"
    ...
    summary = {
        ...
        "endpoint": "/api/v1/classify/security",
        ...
        "note": "Batch performance test for jailbreak/security detection.",
    }
